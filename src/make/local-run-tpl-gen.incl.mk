...
tpl-gen: demand_var-ENV demand_var-ORG demand_var-APP
	cd $(PRODUCT_DIR)/src/python/tpl-gen && poetry run start
...
do-tpl-gen: demand_var-ORG demand_var-ENV demand_var-APP
	docker exec -e ORG=$(ORG) -e APP=$(APP) -e ENV=$(ENV) -e TGT=$(TGT) -e SRC=$(SRC) $(ORG)-${PRODUCT}-tpl-gen-con make tpl-gen

.PHONY: yaml-to-json-file ## @-> render yaml file
yaml-to-json-file: 
	cd $(PRODUCT_DIR)/src/python/tpl-gen && poetry run render
