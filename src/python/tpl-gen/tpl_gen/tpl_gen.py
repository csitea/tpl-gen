...
from yaml_validator import IndividualUsersModel, WireguardPeersModel, TerraformYamlModel
    ...
    for subdir, _dirs, files in os.walk(src):
        for file in files:
            ...
            if current_file_path.endswith(".yaml"):
                print("validating: " + current_file_path)
                ...
                with open(yaml_filename, encoding="utf-8") as file:
                    ...
                    # TODO proper error and type handling, this is for demo purposes
                    if type(cnf) is list:

                        try:
                            WireguardPeersModel(peers=cnf)
                        except:
                            IndividualUsersModel(users=cnf)

                    elif type(cnf) is dict:
                        TerraformYamlModel(**cnf)

                print_success(f"${yaml_filename} is valid")
        ...
        for file in files:
            ...
            if current_file_path.endswith(".yaml"):
                ...
                json_filename = os.path.join(
                    tgt, current_file_path.replace(".yaml", ".json")
                )
...
def set_vars():
    try:
        ...
        if SRC_ == "" or SRC_ is None:
            env_config_dir = os.path.join(
                f"{base_dir}", f"{ORG_DIR_}", f"{ORG_}-infra-conf", f"{APP_}"
            )
        else:
...
def do_generate(ORG_, ENV_, APP_, cnf, tgt_proj_dir):
    ...
    for pathname in [
        os.path.join(tgt_proj_dir, "src", "tpl")
                ...
                if current_file_path.endswith(".tpl"):
                    try:
                        ...
                        with open(
                            ...
                            tgt_file_path = (
                                current_file_path.replace("/src/tpl", "", 1)
                                .replace(".tpl", "")
                                .replace(r"%env%", ENV_)
                                .replace(r"%org%", ORG_)
                                .replace(r"%app%", APP_)
                            ...
                            print(
                                f"START ::: generating tgt_file_path:  {tgt_file_path}"
                            )
